...
listening mode. This fork carries one — `soundcore-bridge` — where upstream has
eight; the shell does not care which is running: they all print the same lines,
read the same commands and end the same four ways. This file is that contract, written once. A
